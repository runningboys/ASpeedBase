...
import com.common.utils.dutil.data.Ranges;
...
public class FileTask implements Runnable {
    ...
    public void run() {
        try {
            ...
            DownloadData data = DownloadManger.getInstance().getDbData(url);
            if (Utils.isFileExists(saveFile) && Utils.isFileExists(tempFile) && data != null && data.getStatus() != Consts.PROGRESS) {
    ...
    private void saveRangeFile() {
        ...
        //Db.getInstance(context).updateProgress(0, 0, Consts.PROGRESS, url);
    ...
    private void startSaveRangeFile(Response response, int index, Ranges range, File saveFile, File tempFile) {
        ...
        try {
            ...
            MappedByteBuffer saveBuffer = saveChannel.map(FileChannel.MapMode.READ_WRITE, range.start[index], range.end[index] - range.start[index] + 1);
    ...
    public Ranges readDownloadRange(File tempFile) {
        ...
        try {
            ...
            MappedByteBuffer buffer = channel.map(FileChannel.MapMode.READ_WRITE, 0, TEMP_FILE_TOTAL_SIZE);
